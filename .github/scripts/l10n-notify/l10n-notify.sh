...
base=$(dirname $(dirname "$script_dir"))/src/content/docs
...
echo "--   └─root   = \"$base/${locale[i]}\""
  ...
  for ori_path in "${abs_paths[@]}";do
    # Get ori_lastUpdated from Git history
    ori_lastUpdated=$(git log -1 --format="%cd" --date=format:"%Y-%m-%d" -- "$ori_path")
    if [ -z "$ori_lastUpdated" ]; then
      ori_lastUpdated=$(date +"%Y-%m-%d")
    fi
    ori_lastUpdated_norm=${ori_lastUpdated//-/}
    ...
    # Get trl_lastUpdated from Git history
    trl_lastUpdated=$(git log -1 --format="%cd" --date=format:"%Y-%m-%d" -- "$trl_path")
    if [ -z "$trl_lastUpdated" ]; then
      trl_lastUpdated=$(date +"%Y-%m-%d")
    fi
    trl_lastUpdated_norm=${trl_lastUpdated//-/}
