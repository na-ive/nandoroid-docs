---
title: Credits
description: Acknowledgements and credits.
---

import { LinkCard, CardGrid } from '@astrojs/starlight/components';

This project is a port and personal creation, built with love and inspired by these amazing developers and projects.

<CardGrid>
  <LinkCard
    title="illogical-impulse"
    description="End-4's Hyprland dotfiles. A lot of the architecture and shell logic here traces back to this."
    href="https://github.com/end-4/dots-hyprland"
  />
  <LinkCard
    title="ii-vynx"
    description="A fork of illogical-impulse by Vynx. Many of the unique features introduced in this fork were a huge inspiration for this project."
    href="https://github.com/vaguesyntax/ii-vynx"
  />
  <LinkCard
    title="end4-pC"
    description="A fork of illogical-impulse by pC. Many of the unique features introduced in this fork were a huge inspiration for this project."
    href="https://github.com/pctrade/end4-pC"
  />
  <LinkCard
    title="Dank Material Shell"
    description="AvengeMedia's DMS. Helped a ton with a lot of the harder parts of the config, and dgop was super useful for system monitoring stuff."
    href="https://github.com/AvengeMedia/DankMaterialShell"
  />
  <LinkCard
    title="Ambxst"
    description="Axenide's Ambxst. Where the notch idea came from, and probably a few other things down the line."
    href="https://github.com/Axenide/Ambxst"
  />
</CardGrid>
